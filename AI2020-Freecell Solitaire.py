...
If you getting package errors when running it as a script,
make sure that these packages are installed in the same directory as Python
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
"""
...
if(run_as_script):
    ...
    pseudoargv=['AI2020-Freecell Solitaire.py','depth','INSERT PATH HERE','output.txt']
    ARGV=pseudoargv
...
if(run_as_script):
    ...
    path=ARGV[2]
    inputs = pd.read_csv(r'INSERT PATH HERE', sep=" ", header=None)
    ...
    def freecells2tableau(self,TStNum): #check whatever card you can add to  tableau from freecells 
        if(self.tableau[TStNum]): #if the stack is not empty
            ...
            for pos in range(len(self.free_cells)): 
                if(int(self.free_cells[pos].number)==-1): #if its empty go to the next card 
                    continue
                else:
...
def syntax_message():
    	print("solitaire <method> <input-file> <output-file>\n\n")
    	print("where: ")
